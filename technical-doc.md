...
| Outbound CCTP delivery (Stellar→EVM) has no automatic relay on either testnet or mainnet — confirmed against Circle's own technical guide (developers.circle.com/cctp/references/technical-guide: "An API consumer must query this attestation and submits it onchain to the destination domain's MessageTransmitterV2#receiveMessage function"; no testnet/mainnet distinction drawn anywhere in that doc, no mention of Circle operating a relayer). This is architectural, not a testnet-only gap: third-party aggregators (LI.FI, Squid) and Wormhole's own CCTP integration each run their own separate relay service specifically because Circle's own infrastructure doesn't submit `receiveMessage` on a sender's behalf. Confirmed directly on testnet (widget phase, 2026-09-12): a real, fully attested burn sat undelivered for ~46 minutes with zero automatic submission, until manually completed by calling `receiveMessage` with the real message/attestation bytes from a funded scratch EVM account. | Widget shows "verified, delivering" indefinitely with no path to "delivered" unless *someone* — the sender, the recipient, or a future Ferryline-operated outbound relayer — submits `receiveMessage` and pays its gas. Not a fund-loss risk (the attestation and mint recipient are real and correct the whole time; CCTP's design makes `receiveMessage` permissionless, so anyone, including the recipient themselves, can complete it with a small amount of destination-chain gas) — but a real UX/completeness gap: outbound CCTP is not "fire and forget" today the way inbound (via Ferryline's own relayer) is. Severity: does not block the flow from being genuinely usable (any wallet or script can complete delivery permissionlessly, same as any other CCTP integrator's users can), but it is not production-complete without either documentation telling integrators/users this step exists, or a dedicated outbound relayer service (mirroring the inbound one) built in a future phase — building that relayer is explicitly out of scope for the widget phase that discovered this. | Widget's own outbound CCTP status display must not claim or imply "delivered" without a real, observed `receiveMessage` consumption on the destination chain (already true: `track()` only advances to `delivered` on a real `usedNonces` check) — but its "verified, delivering" state should carry an honest caveat that this step may require manual completion, rather than reading as an ETA-bound in-progress status. | Time-in-"verified" per outbound transfer, alerting on transfers stuck past a threshold (e.g. 1 hour) with no destination-chain nonce consumption |
