...
//
// Why this exists: `build`, `typecheck`, `test`, `clean`, and the two `record:*` fixture scripts
// are real, working scripts when run from this package's own source checkout inside the monorepo
// (Turbo and this repo's CI genuinely invoke them by these exact names — removing them from the
// SOURCE package.json would break the real dev/CI pipeline). But none of them work post-install:
// the published tarball ships only `dist/`, `CHANGELOG.md`, `LICENSE`, and `package.json` — no
// `tsconfig.json`, no `tsup.config.ts`, no `vitest.config.ts`, no `src/`, no `scripts/`. Verified
// directly, each one, against a real extracted tarball before writing this script:
