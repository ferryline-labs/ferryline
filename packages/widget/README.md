...
| `prepare-step-poll-max-attempts` | a positive integer               | `20`            | Real, public config for the poll that replaces the old fixed-timer wait before the deferred CCTP burn step — see `src/client.ts`'s `pollPrepareStep` doc comment for the full incident writeup.                                                |
| `prepare-step-poll-interval-ms`  | milliseconds, a positive integer | `1500`          | Same poll as above. An invalid or non-positive value on either attribute falls back to its real default.                                                                                                                                       |
