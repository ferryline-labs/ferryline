...
| `prepare-step-poll-max-attempts` | a positive integer               | `20`            | See [Polling before the deferred burn step](#polling-before-the-deferred-burn-step) below. Raise it if your own RPC node has worse allowance-visibility lag than this project's testing observed.       |
...
(see [Attributes](#attributes) above) are the real, public config for this poll's bound and
interval; both are live-validated against a real testnet transfer, not just unit-tested — see
[the real, dated experiment record](https://github.com/ferryline-labs/ferryline/blob/main/packages/core/verified/experiments/2026-09-16-widget-poll-prepare-step-live.md)
