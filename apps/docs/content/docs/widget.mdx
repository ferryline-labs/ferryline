...
verbatim (only re-indented; its own inline comment is trimmed here — the full version explains the
historical double-registration bug this exact `isFinalStep` check exists to prevent, see that
...
The widget's own delivery-status copy reflects both cases. It renders one of two real strings
depending on whether `relayer-url` is set, from its own `outboundDeliveryCaveat(relayerConfigured)`
helper — not one static "just wait" message regardless of configuration.
