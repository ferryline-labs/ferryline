...
condition, so `require("@ferryline/sdk")` from a CommonJS file fails with
`ERR_PACKAGE_PATH_NOT_EXPORTED` rather than working. This is a real, intentional choice (matching
`@ferryline/core`/`@ferryline/widget`), not a bug — `import` it, or use dynamic `import()` from
...
UI before this existed). `@ferryline/widget` calls this automatically on your behalf once its final
...
[end-to-end walkthrough](/docs/bridge-walkthrough)).
