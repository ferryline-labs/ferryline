...
`relayerUrl`/`relayerApiKey` are read by [`registerOutboundTransfer()`](#registeroutboundtransfer)
below, which `POST`s to `${relayerUrl}/outbound-transfers`. Registering an **inbound** EVM→Stellar
transfer with the relayer is a separate call, `POST /transfers`, documented on the
[Relayer](/docs/relayer) page — the SDK doesn't wrap that one for you the way it now wraps the
...
```ts
registerOutboundTransfer(transferId: TransferId): Promise<void>;
```

Opts an **outbound** (Stellar→EVM) CCTP transfer into automatic delivery via a self-hosted
[relayer](/docs/relayer). The real doc comment on this method is exact about timing, quoted in
full because getting this wrong is a real bug this project already shipped and had to fix:
...
> calling it earlier is a REAL BUG THIS PROJECT ALREADY SHIPPED AND HAD TO FIX. Use
> `isFinalStep(built, stepIndex)` (exported below) to find the right point:
> `if (isFinalStep(built, stepIndex)) { await ferryline.registerOutboundTransfer(built.transferId); }`,
> called only once, after that specific step's real tx hash has already been recorded via
> `markSubmitted`.

Concretely, it `POST`s to `${relayerUrl}/outbound-transfers` with
...
`POST /transfers` already has (see [Relayer](/docs/relayer)). It **never throws**: three separate
conditions all make it a silent no-op rather than an error — no `relayerUrl` configured; the
transfer isn't an outbound CCTP transfer (checked via `rail === "usdc-cctp" && request.from.chain
=== "stellar"`); or the HTTP registration itself fails (network error, non-`201` response) — that
case is caught, logged with `console.error`, and swallowed, on the reasoning that the burn is
already on-chain and unaffected by a failed registration call. `@ferryline/widget` calls this
automatically on your behalf once its final step confirms, so a raw SDK integrator needs it only
when driving transfers directly (see the [end-to-end walkthrough](/docs/bridge-walkthrough)).
...
`stepIndex === built.steps.length - 1`. The canonical way to find the one correct call site for
...
(`version.js` — `SDK_VERSION` mirrors `package.json`'s version, currently `"0.1.0"` (the first published version — `@ferryline/sdk` and `@ferryline/core` are the only two packages published to npm so far), with a test
...
package's HTTP API, not part of `@ferryline/sdk`. There's no `registerInboundTransfer` method on
`Ferryline` either; that one lives only on `@ferryline/widget`'s custom element (see
[Widget](/docs/widget)), which wraps the relayer's inbound endpoint for you. `@ferryline/sdk`
