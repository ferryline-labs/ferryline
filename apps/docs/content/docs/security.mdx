...
Rather than repeat a number from an older document, these counts come from running each package's
test suite directly, on 2026-09-14:

| Package             | Command                                | Result                      |
| ------------------- | -------------------------------------- | --------------------------- |
| `@ferryline/core`   | `pnpm --filter @ferryline/core test`   | 60 tests passing (4 files)  |
| `@ferryline/sdk`    | `pnpm --filter @ferryline/sdk test`    | 83 tests passing (8 files)  |
| `ferryline-relayer` | `pnpm --filter ferryline-relayer test` | 98 tests passing (10 files) |
| `@ferryline/widget` | `pnpm --filter @ferryline/widget test` | 34 tests passing (4 files)  |

That's **275 passing tests**, confirmed fresh, across the four TypeScript packages — matching
`ARCHITECTURE.md`'s own "275 passing across four TypeScript packages" figure for unit and
component tests. Two things worth being precise about:

- The relayer's `test` script deliberately excludes its live-database integration tests
  (`--exclude '**/*.integration.test.ts'`); those 18 tests run against a real, disposable Postgres
  container and need `docker compose up -d postgres` first (see the [relayer page](/docs/relayer)).
  They weren't re-run for this page, so treat "98" as the freshly-confirmed unit/component number
  and the relayer's full count as 98 + 18 documented elsewhere, not independently re-verified here.
- The router's 29 Soroban/Rust tests (`cargo test`, in its own Cargo workspace) also weren't re-run
  for this page — this page's test-verification step ran only the four `pnpm` commands above, not
  `cargo test`. The 29 figure is carried from `ARCHITECTURE.md` and `README.md`, not independently
  confirmed fresh here.
