...
[relayer page](/docs/relayer) for how to run them with a real, disposable Postgres container. An
...
- **`@ferryline/ui` and `apps/playground` were both missing from this table entirely** until this
  pass, despite being real, tested packages already — the same class of staleness as a wrong
  number, just by omission instead. Ten packages/apps have real test suites as of this writing; if
  an eleventh is added without this table growing to match, that is the same gap recurring.
