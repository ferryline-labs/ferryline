...
An earlier version of this page repeated figures from `README.md`/`ARCHITECTURE.md` that turned
out to be stale — real Phase 6 test files roughly doubled the SDK's and relayer's counts without
those headline numbers being updated. So rather than repeat a number from another document, every
count below comes from actually running each package's real test suite, on 2026-09-14:

| Package                    | Command                                       | Result                       |
| -------------------------- | --------------------------------------------- | ---------------------------- |
| `@ferryline/core`          | `pnpm --filter @ferryline/core test`          | 60 tests passing (4 files)   |
| `@ferryline/sdk`           | `pnpm --filter @ferryline/sdk test`           | 91 tests passing (8 files)   |
...
That's **444 passing tests**, all independently re-run for this page, not carried from another
document. Two things worth being precise about, disclosed rather than smoothed over:

- The relayer's `test` script deliberately excludes its live-database integration tests
  (`--exclude '**/*.integration.test.ts'`); those need a real, disposable Postgres container
  (`docker compose up -d postgres`, see the [relayer page](/docs/relayer)) that wasn't available
  in the environment this page was verified from (no running Docker daemon). Real evidence points
  to roughly 37 of them now — up from an earlier 18, doubled by the same real inbound/outbound
  split behind the relayer's 98→196 unit-test growth above — but this page did not independently
  re-run them, so treat 37 as a well-evidenced estimate, not a confirmed count the way the 444
  above are.
- **A real discrepancy in the project's own root docs, found while verifying this page, not fixed
  here:** `README.md`'s current headline ("353 automated tests… 116 relayer including
  live-database integration tests") and `ARCHITECTURE.md`'s equivalent figures don't reflect the
  real current counts above — they were written before Phase 6's real test additions and haven't
  been updated since. This page's own numbers are independently re-run and real; fixing the root
  project docs' headline figures is out of scope for this docs site to do unilaterally, but is
  exactly the kind of stale-claim fix [Contributing](/docs/contributing)'s Documentation section
  welcomes.
