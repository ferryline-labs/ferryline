...
documents, not generic open-source boilerplate. Where an answer is short on purpose, it links to the
page that actually works through the reasoning — see [Core Concepts](/docs/core-concepts) especially.
...
No. There's no `@fastify/cors` dependency in `packages/relayer/package.json`, and grepping the whole
repository for `cors` (case-insensitive) turns up nothing at all — no CORS plugin is registered
anywhere in the relayer's Fastify app. If you're calling the relayer's HTTP API directly from a
browser, plan around that rather than assuming it's handled. See [`/docs/relayer`](/docs/relayer).

## What happens to an outbound Stellar→EVM transfer after it's attested — does it deliver itself?

No. CCTP's `receiveMessage` call on the destination chain is permissionless by the protocol's own
design, meaning anyone can submit it, but nothing in this pipeline does so automatically for the
Stellar→EVM direction: not Circle's own infrastructure, and not this project's relayer, which only
covers the opposite (EVM→Stellar) direction. `track()` will keep reporting `verified` (attested, not
yet delivered) indefinitely until someone — the sender, the recipient, or their own infrastructure —
submits that call themselves. This doesn't put funds at risk (the attestation and mint recipient are
correct the whole time), but it does mean the transfer doesn't complete on its own. See
[Core Concepts](/docs/core-concepts#three-problems-solved-once) for the full explanation, and
`packages/widget/e2e/submit-receive-message.mjs` for a real, working example of submitting that call
manually.
...
real starting points in [Contributing](/docs/contributing#where-to-start).
