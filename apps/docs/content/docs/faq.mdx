...
(`POST`/`DELETE /admin/api-keys`, gated by a separate `FERRYLINE_ADMIN_SECRET`) for creating and
revoking a real key without needing direct database access — see the relayer's own README's
"Admin: issuing and revoking real integrator API keys" section. See [`/docs/relayer`](/docs/relayer).
