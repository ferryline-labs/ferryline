...
between the four named in the [Introduction](/docs):

```mermaid
flowchart LR
  Widget["@ferryline/widget<br/>&lt;ferryline-widget&gt;"] -->|"uses"| SDK["@ferryline/sdk<br/>Ferryline class"]
  SDK -->|"quote / build / track"| Rails["CCTP + USDT0 OFT<br/>rail contracts"]
  SDK -.->|"registerOutboundTransfer()<br/>(opt-in)"| Relayer["ferryline-relayer<br/>self-hosted"]
  Widget -.->|"registerInboundTransfer()"| Relayer
  Relayer -->|"watches attestation,<br/>submits the completing call"| Rails
  OtherContract["Another Soroban contract<br/>(a vault, payroll, escrow — not Ferryline's)"] -->|"send_cross_chain(_batch)"| Router["ferryline-router"]
  Router -->|"dispatches to"| Rails
```
...
_different_ caller entirely: another Soroban contract (a vault, a payroll contract, anything with
