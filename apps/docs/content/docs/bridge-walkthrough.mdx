...
flowchart TD
  A["quote()"] --> B["build()"]
  B --> C{"steps.length"}
  C -->|"2 — needs approve"| D["sign + submit approve"]
  D --> E["prepareStep(id, 1)"]
  E --> F["sign + submit burn"]
  C -->|"1 — allowance already covers it"| F
  F --> G["markSubmitted(id, burnTxHash)"]
  G --> H{"relayer-url configured?"}
  H -->|"yes"| I["registerOutboundTransfer(id)<br/>on the final step only"]
  H -->|"no"| J["track() — verified, then stuck"]
  I --> K["track() — verified, then delivered<br/>(relayer submits receiveMessage)"]
  J --> L["manual receiveMessage<br/>(anyone, permissionless)"]
```
...
watches for it and completes the mint. The real step sequence, matching the code below exactly:

```mermaid
flowchart TD
  A["encodeDepositForBurnWithHookToStellar()"] --> B["sendTransaction — real burn on Sepolia"]
  B --> C["POST /transfers { sourceTxHash }<br/>no amount, no recipient"]
  C --> D["poll GET /transfers/:id"]
  D --> E{"status"}
  E -->|"pending"| D
  E -->|"attested"| D
  E -->|"delivered"| F["done — real mint_and_forward<br/>landed on Stellar"]
```
