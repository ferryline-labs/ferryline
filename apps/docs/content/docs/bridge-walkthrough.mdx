...
Here's the part worth knowing before you rely on this in a real product: **`track()`'s `delivered`
stage never arrived on its own.** `track()` only _observes_
`MessageTransmitterV2.usedNonces(nonce)` on the destination chain — it never submits the
`receiveMessage` call itself, and neither does anything else in this project for the outbound
direction (Ferryline's own relayer exists only for the opposite, inbound direction — see
[the Relayer docs](/docs/relayer)). In the real run above, the nonce was still unconsumed after 40
real minutes of polling. Circle's own documentation says an API consumer must submit the
attestation onchain themselves; nothing does that automatically here for outbound transfers today.

CCTP's `receiveMessage` is permissionless by design — anyone holding the real message and
attestation bytes can submit it and pay the gas. Fetching those bytes and submitting it directly,
using the SDK's own exported ABI:
...
This direction _does_ have automatic delivery, via the self-hosted relayer (see
