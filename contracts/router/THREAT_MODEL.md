...
was deferred, non-blocking scope through STEP 8; it is implemented and tested as of STEP 9. See
each row for the full reasoning.
...
| **Repud.1 — No repudiation source for what actually executed**                                                  | If the router does not emit a structured event per successful `send_cross_chain`/batch leg (payer, rail, destination, amount, the exact args passed to `require_auth`), there is no independent record to reconstruct what happened for a disputed transfer, and the roadmap's own risk register (`technical-doc.md`, "Router event stream") already commits to this as a mitigation.                                                                                                                                                                                                                                                                                                                                                                              | **Implemented and tested as of STEP 9.** `dispatch_one_leg` publishes a `TransferSent` event (payer/rail as topics, destination/amount as data) at the same choke point and under the same Elev.2 ordering as the `Volume` write — see STEP 9 below for the full account and `src/invariants.rs`'s `repud_1_*` tests plus the extended `elev_2_volume_write_is_lexically_after_the_external_call_in_source` for the proof. The aggregate `volume()` counter (Dos.2) still attests total volume moved; `TransferSent` is the new per-transfer record this row asked for.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                           |
...
## STEP 9 — Repud.1 implemented: per-leg `TransferSent` event (2026-09-13)

**Repud.1 is CLOSED.** `dispatch_one_leg` now publishes a `TransferSent` event (payer and rail as
`#[topic]` fields, destination and amount as data) at the same choke point, under the same
Elev.2 ordering guarantee, as the existing `Volume` counter write — reached only once a leg's
rail call has actually returned successfully, for both `send_cross_chain` and
`send_cross_chain_batch` (both funnel every leg through this one function). "The exact args
passed to `require_auth`" (`Repud.1.R.1`'s fourth listed field) is not a separate field on the
event: `require_auth()`'s own semantics already authorize the current invocation's full arguments
(payer, rail, dest, amount — see `send_cross_chain`'s own doc comment), so those ARE the
authorized args, not a distinct fact to record twice under a different name.

**Verification: 3 new tests, plus one existing test extended, 32/32 total, fmt/clippy clean, real
`stellar contract build` succeeds** (`src/invariants.rs`):

- `repud_1_single_send_emits_transfer_sent_with_the_real_call_s_own_values` — a single send emits
  exactly one event, with that call's real payer/rail/destination/amount, checked against the
  event's own real XDR encoding (`Event::to_xdr`), not a loose field-by-field guess.
- `repud_1_batch_emits_one_transfer_sent_per_leg_not_one_per_batch` — a two-leg batch emits TWO
  events, each carrying that leg's own values — directly disproving the plausible-but-wrong
  alternative reading "one aggregate event per top-level call."
- `repud_1_a_rolled_back_leg_emits_no_event_even_for_an_earlier_valid_leg_in_the_same_batch` — a
  batch where leg 2 fails emits ZERO events, even though leg 1 was individually valid and had
  already reached its own success path.
- `elev_2_volume_write_is_lexically_after_the_external_call_in_source` (pre-existing, now
  extended) — a structural, source-text-position check that the event publish call, like the
  `Volume` write, appears lexically after every real external-call site in `dispatch_one_leg`.

**A real finding from this STEP's own mutation-testing pass, worth recording rather than treating
as an unstated assumption**: deliberately moving the event-publish call to the TOP of
`dispatch_one_leg` (before rail dispatch, unconditionally) did NOT make the rollback test above
fail — Soroban's own transaction semantics discard a reverted call's published events the same
way they discard its storage writes, so that behavioral test alone cannot distinguish "correctly
placed" from "wrongly placed but rolled back anyway" (it DOES produce a real, different bug —
a duplicate event per leg — caught instead by the batch test above). The structural,
source-position test (extended in this STEP) is what actually closes that specific gap; see
`repud_1_a_rolled_back_leg_emits_no_event_...`'s own doc comment in `src/invariants.rs` for the
full account, in the same "corrected finding" style already established for a prior mutation-test
result on the `Volume` write (see this file's own note near that test).

**Explicit dependency this closes**: the roadmap's metrics page / adoption-milestone verification
work (flagged in the Repud.1 row below and in CONTRIBUTING.md) can now depend on per-transfer
on-chain data — `volume()` was always aggregate-only; `TransferSent` is the first per-transfer
record.
